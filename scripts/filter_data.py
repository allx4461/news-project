from src.config import CHUNKSIZE, USECOLS, RAW_DATA_PATH, PROCESSED_DATA_PATH
...
from src.config import RAW_DATA_PATH, CHUNKSIZE,TICKER,USECOLS
...
def filter_ticker_streaming(
    ...
    """чанками грузит данные по только нужному тикеру в csv-формате, оставляя только usecols колонки из конфига. 
    также удаляет строки без даты или названия статьи и чистит лишние пробелы в строковых колонках"""
    ...
    for chunk in chunks:
        ...
        ticker_chunk = chunk[
            chunk["Stock_symbol"].astype(
                "string").str.upper().eq(ticker.upper())
        ...
        ticker_chunk["Article_title"] = ticker_chunk["Article_title"].astype(
            str).str.strip()
        ticker_chunk = ticker_chunk[ticker_chunk["Article_title"] != ""]#дропна на пустые строки не триггерится. доп проверка
